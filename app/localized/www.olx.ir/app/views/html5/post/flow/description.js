...
module.exports = Base.extend({
    ...
    validate: function() {
        ...
        if ($title.val().length < 10) {
            ...
            $title.addClass('error').after('<small class="error">' + translations.get(this.app.session.get('selectedLanguage'))['misc.TitleCharacters_Mob'].replace('<<NUMBER>>', ' ۰۱ ') + '</small>');
            statsd.increment([location, 'posting', 'invalid', this.app.session.get('platform'), 'title']);
        }
        if ($description.val().length < 10) {
            failed = true;
            this.$el.addClass('error');
            $description.addClass('error').after('<small class="error">' + translations.get(this.app.session.get('selectedLanguage'))['misc.DescriptionCharacters_Mob'].replace('<<NUMBER>>', ' ۰۱ ') + '</small>');
